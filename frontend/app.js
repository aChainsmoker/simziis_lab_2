...
async function logout() { try { await api('/api/auth/logout',{method:'POST'}); } catch(e) {} $('app-section').hidden=true; $('auth-section').hidden=false; }
async function openApp() { try { const user=await api('/api/auth/me'); $('current-user').textContent=`Пользователь: ${user.username}`; $('auth-section').hidden=true; $('app-section').hidden=false; selectTab('public-data'); } catch(e) { show(e.message); } }
