const tokenKey = 'lab2_access_token';
...
async function register() { const dataInput=credentials(); if(!dataInput) return; try { const data=await api('/api/auth/register',{method:'POST',body:JSON.stringify(dataInput)}); localStorage.setItem(tokenKey,data.access_token); await openApp(); } catch(e) { show(e.message); } }
async function login() { const dataInput=credentials(); if(!dataInput) return; try { const data=await api('/api/auth/login',{method:'POST',body:JSON.stringify(dataInput)}); localStorage.setItem(tokenKey,data.access_token); await openApp(); } catch(e) { show(e.message); } }
async function logout() { try { await api('/api/auth/logout',{method:'POST'}); } catch(e) {} localStorage.removeItem(tokenKey); $('app-section').hidden=true; $('auth-section').hidden=false; }
async function openApp() { try { const user=await api('/api/auth/me'); $('current-user').textContent=`Пользователь: ${user.username}`; $('auth-section').hidden=true; $('app-section').hidden=false; selectTab('public-data'); } catch(e) { localStorage.removeItem(tokenKey); show(e.message); } }
